...
# Run daemon manually
uv run quin-core                        # foreground; Ctrl+C to stop
QUIN_PORT=8000 uv run quin-core        # override port
...
This is a **dual-process** local AI agent system. `quin-core` is a persistent daemon; `quin` and `quin-tui` are clients that connect to it over a Unix domain socket.

```
quin-core (daemon)
  └─ listens on 127.0.0.1:7437 (TCP)
       ↑ JSON-RPC 2.0 NDJSON
quin (CLI)   quin-tui (TUI, S2+)
```

**`quin-tui` is the primary frontend.** All user-facing work on task management, observability, and interaction should be designed for and validated in the TUI first. The `quin` CLI exists only for quick scripted testing and debugging — it is not a product surface. When implementing features that touch the user interface, invest in the TUI layout, event rendering, and keyboard interactions. Do not shortcut TUI work by pointing to the CLI as an alternative.

### Protocol layer (`src/quin_claude/core/bus/`)
...
### Transport layer (`src/quin_claude/core/transport/`)
...
### Config (`src/quin_claude/core/config.py`)

Four-tier priority: **built-in defaults → `~/.quin/config.toml` → `.env` → env vars**.
...
Relevant env vars: `QUIN_CONFIG`, `QUIN_HOST`, `QUIN_PORT`, `QUIN_LOG_LEVEL`, `QUIN_LOG_FILE`, `QUIN_LOG_FORMAT`.

### Daemon entry (`src/quin_claude/core/app.py`)
...
Integration tests in `tests/conftest.py` spawn a real daemon subprocess using a random free port (via `free_port` fixture). The fixture finds a free port, releases it, passes it to the daemon via `QUIN_PORT`, then polls `asyncio.open_connection` until the daemon is ready.
