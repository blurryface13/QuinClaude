...
import quin_claude
from quin_claude.core.bus.commands import PongResult
from quin_claude.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from quin_claude.core.config import QuinConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: QuinConfig) -> None:
    try:
...
async def _ping(config: QuinConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{quin_claude.__version__}"},
    }
