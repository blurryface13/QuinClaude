...
import quin_claude
from quin_claude.core.bus.commands import (
    AgentRunCommand,
    AgentRunResult,
...
from quin_claude.core.bus.envelope import EventPushEnvelope
from quin_claude.core.config import QuinConfig, get_config
from quin_claude.core.events.bus import EventBus
from quin_claude.core.llm.provider import create_provider
from quin_claude.core.logging_setup import setup_logging
...
from quin_claude.core.permissions.storage import load_policy_file
from quin_claude.core.runner import AgentRunner
from quin_claude.core.runs import events_file, new_run_id
from quin_claude.core.session import SessionManager, SessionStore
from quin_claude.core.trace.record import TraceRecord
from quin_claude.core.trace.writer import TraceWriter
from quin_claude.core.transport.ipc_broadcaster import IpcEventBroadcaster
from quin_claude.core.transport.socket_server import SocketServer, get_connection_writer
...
class CoreApp:
    def __init__(self) -> None:
        ...
        self._config: QuinConfig | None = None
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=quin_claude.__version__,
    ...
    async def _replay_events(
        ...
        if not path.exists():
            for candidate in Path("~/.quin/sessions").expanduser().glob(
    ...
    async def run(self) -> None:
        ...
        policy_file = Path("~/.quin/policy.toml").expanduser()
        ...
        sessions_root = Path("~/.quin/sessions").expanduser()
        ...
        logger.info("quin-core %s listening addr=%s", quin_claude.__version__, addr)
