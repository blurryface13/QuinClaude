...
from quin_claude.core.skills.loader import SkillLoader
from quin_claude.core.transport.socket_client import IpcError, SocketClient
...
class QuinTuiApp(App[None]):
    """QuinClaude TUI：终端滚屏风格，实时展示 agent 执行过程。"""
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]QuinClaude[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]QuinClaude[/bold]  [dim]{self._host}:{self._port}[/dim]"
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update(f"[bold]QuinClaude[/bold]  [red]subscribe error: {e}[/red]")
            finally:
...
# TUI 入口：读取配置并启动 QuinTuiApp
def run(config: QuinConfig, replay_run_id: str | None = None) -> None:
    app = QuinTuiApp(config.host, config.port, replay_run_id=replay_run_id)
    app.run()
