...
cp .env.example .env          # 配置 LLM Provider / API Key
uv run quin ping              # 探活
uv run quin run --goal "..."  # 下发目标运行 Agent
uv run quin-tui               # 启动终端 UI（主要前端）
```
...
> 需要真实 API 的集成用例默认会执行；仅运行离线用例可执行
> `uv run pytest -m "not integration"`。

## 许可证

本项目基于上游 [KamaClaude](https://github.com/youngyangyang04/KamaClaude) 二次开发，沿用其 [MIT License](LICENSE)。
