...
from quin_claude.core.llm.provider import AnthropicProvider
from quin_claude.core.llm.types import LlmResponse
...
# 功能：验证缺少 ANTHROPIC_API_KEY 时 provider 调用报普通运行时错误，不杀掉 daemon 进程
# 设计：provider 可先被 daemon 装配；真正调用模型时才失败，并由 runner 捕获为 llm_error
